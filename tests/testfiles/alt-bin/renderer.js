...
const FONT_STYLE_TO_CSS = {
    [FontStyle.Italic]: 'font-style: var(--shiki-font-style, italic)',
    [FontStyle.Bold]: 'font-weight: var(--shiki-font-weight, bold)',
    [FontStyle.Underline]: 'text-decoration: var(--shiki-font-underline, underline)'
}
...
const renderToHtml = function(lines, options = {}) {
    ...
    html += `<pre class="${className}" style="background-color: var(--shiki-color-background, ${bg})">`
    if (options.langId) {
    ...
    lines.forEach((l, index) => {
        ...
        l.forEach(token => {
            const cssDeclarations = [`color: ${token.color || 'var(--shiki-color-text, '+options.fg+')'}`]
